...
- `./scripts/run-m5-validation.sh`: all nine `(batch, columns)` variants
  ...
  selected `(16,4)` median wall time `1.379001 ms` in this run. This is a raw
...
Do not start M7. The inherited physical M1–M5 validation suite has now been
rerun and passed. Next, select and independently license-review a permissive
