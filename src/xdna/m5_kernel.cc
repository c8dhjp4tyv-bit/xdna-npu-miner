...
constexpr uint32_t kItemErrorNone = 0U;
constexpr uint32_t kItemErrorInvalidMode = 1U;
...
constexpr uint32_t kOutputErrorOffset = 124U;
constexpr uint32_t kOutputMagic = 0x3152354DU;
                  ...
                  uint32_t error)
{
    ...
    write_u32(output, kOutputErrorOffset, error);
}

} // namespace

// The host invokes this kernel once for each item acquired by one lane. The
// lane placement and batch scheduling are expressed in m5_program.py; this
// function owns only the deterministic, device-side M4 window semantics.
extern "C" void bpp9000_m5_dispatch(const uint8_t* device_input, uint8_t* output)
{
    ...
    uint32_t error = kItemErrorNone;

    if (mode != kModeWindowScore || input_rows != window_width || output_neuron >= kPopulation
        || signal_neuron >= kPopulation || output_neuron == signal_neuron) {
        status = kStatusTimeout;
        score = kTimeoutScore;
        error = kItemErrorInvalidMode;
    } else {
    ...
    write_result(output, current, score, status, ticks, feed_count, predicted, expected, error);
}
